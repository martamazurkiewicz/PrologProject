...
below(X,Y,S):-above(Y,X,S).
floorBelow(X,Y,S):-floorAbove(Y,X,S).
left(X,Y,order(X,Y,_,_,_,_)).
left(X,Y,order(_,_,X,Y,_,_)).
left(X,Y,order(_,_,_,_,X,Y)).
nextTo(X,Y,S):-left(X,Y,S).
nextTo(X,Y,S):-left(Y,X,S).
groundF(X,order(_,_,_,_,X,_)).
groundF(X,order(_,_,_,_,_,X)).
secondF(X,order(X,_,_,_,_,_)).
secondF(X,order(_,X,_,_,_,_)).
between(X,Y,Z,S):-above(X,Y,S),above(Y,Z,S).
between(X,Y,Z,S):-above(Z,Y,S),above(Y,X,S).

clue1(S):-above(student(stanislaw,_,_),student(_,_,przegladaCzat),S).
clue2(S):-below(student(_,rano,_),student(_,_,zmywaNaczynia),S).
clue3(S):-nextTo(student(miroslaw,_,_),student(_,wCzasieWolnym,_),S).
clue4(S):-nextTo(student(_,_,odrabiaLekcje),student(ania,_,_),S).
clue5(S):-between(student(ania,_,_),student(_,wCzasieZajec,_),student(adam,_,_),S).
clue6(S):-secondF(student(_,wNocy,_),S).
clue7(S):-groundF(student(adrian,_,_),S).
clue8(S):-floorAbove(student(ola,_,_),student(_,_,jeWarzywa),S).
clue9(S):-floorBelow(student(_,_,wysylaZdjecia),student(_,_,przegladaCzat),S).
clue10(S):-nextTo(student(_,wieczorem,_),student(adam,_,_),S).
clue11(S):-nextTo(student(_,rano,jeWarzywa),student(_,_,_),S).
clue12(S):-nextTo(student(_,poPoludniu,zmywaNaczynia),student(_,_,_),S).
clue13(S):-nextTo(student(miroslaw,_,_),student(_,_,przegladaCzat),S).
clue14(S):-nextTo(student(_,wCzasieWolnym,przegladaCzat),student(_,_,_),S).
